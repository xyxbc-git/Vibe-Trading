...
export default function Header() {
  ...
  return (
    <header className="h-12 flex items-center justify-between pl-20 pr-6 bg-jarvis-card border-b border-jarvis-border select-none" style={{ WebkitAppRegion: "drag" } as React.CSSProperties}>
        ...
        {wallet && (
          <span className="text-sm font-mono text-jarvis-text-secondary">
            ...
            <span className="text-jarvis-text">
              ${Number((wallet as Record<string, unknown>)?.cash_usdt ?? (wallet as Record<string, unknown>)?.cash ?? 0).toLocaleString("en-US", { minimumFractionDigits: 2 })}
            </span>
          </span>
        )}
