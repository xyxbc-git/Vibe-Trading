// Phase D-3 · structured-feature online model. Ported from
// frontend/src/lib/drawingModel.ts.
//
// A tiny online logistic regression that learns, from market-context features
// extracted at sample time (volatility, trend slope, range position, touch
// evidence), whether a drawing tends to be "respected" (hitRate >= 0.5) in
// that regime. Its prediction is a third reliability signal, blended
// conservatively and gated by sample count.
//
// Everything here is pure + JSON-serialisable so it is fully unit-testable and
// can be re-trained from the accumulated results log on load.
...
// Minimum featured samples before the structured model may influence
// reliability. Desktop samples accumulate one per closed bar per active mode
// (the log dedupe key is the last bar's timestamp), so with e.g. 3 modes
// active this gate is reached after ~5 new bars.
export const MODEL_MIN_SAMPLES = 15;
...
// Build a training set from accumulated samples. Only samples that were
// actually tested (touches > 0) carry signal — a line the future never touched
// is neither "respected" nor "failed". Label = respected on majority of touches.
