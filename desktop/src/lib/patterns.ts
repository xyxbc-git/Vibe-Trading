// Classic chart-pattern recognition for the desktop K-line chart.
//
// Ported from frontend/src/lib/patterns.ts — the detection engine is kept
// verbatim (pure functions over OHLC arrays, zero renderer deps). The only
// difference vs. the web version: the echarts markLine/markArea overlay
// builder is NOT here — the desktop render payload (DrawingResult segments/
// hlines + StructureMarker touch points) lives in ./patternOverlay.ts.
//
...
export interface PatternKeyPoint {
  ...
  ts: string; // dates[index]（桌面端为 unix 秒字符串，展示层负责格式化）
  price: number;
