// 形态识别引擎测试 —— 与 frontend/src/lib/__tests__/patterns.test.ts 同源
// （引擎为逐行移植，判定要件一致；echarts overlay 部分桌面端不存在，
// 渲染载荷测试见 patternOverlay.test.ts）。

import { describe, it, expect } from "vitest";
import { detectPatterns, PATTERN_NAMES, type DetectedPattern } from "../patterns";
...
function barsFromAnchors(anchors: [number, number][]): BaseData {
  ...
  return {
    // 桌面端 dates 为 unix 秒字符串（Chart.tsx 用 String(candle.time) 生成）
